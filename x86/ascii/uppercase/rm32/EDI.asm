...
; This decoder requires EDI to point to its base ("start")
...
; DECODE 0x6B FOR IMUL INSTRUCTION
    XOR     [BYTE EDI+EAX*2 - eax_value*2 + imul_offset], EDX
; DECODE 0x75 FOR JNE INSTRUCTION
    XOR     [BYTE EDI+EAX*2 - eax_value*2 + jnz_offset], EDX
...
; bytes will be "patched" before the loop runs to re-create the right bytes.
    ...
    ;IMUL EAX, [EDI+EDX*2 - (eax_value+1)*2 + data_offset], 30         ; JNZ offset EF ; Terminating 00
imul: db 0x6B^code, 0x44, 0x57, - (eax_value+1)*2 + data_offset, 0x30  ; 4A * 30 -> E0 ; 30 * 30 -> 00
imul_offset equ imul - start
    XOR     AL, [BYTE EDI+EDX*2 - (eax_value+1)*2 + data_offset + 1]   ; E0 ^ 45 -> A5 ; 00 ^ 45 -> 45
    XOR     [BYTE EDI+ESI - (esi_value+1) + data_offset], AL           ; 4A ^ A5 -> EF ; 45 ^ 45 -> 00 
