...
; This decoder requires EDX to point to its base ("start")
...
; DECODE 0x6B FOR IMUL INSTRUCTION
    XOR     [BYTE EDX+EAX*2 - eax_value*2 + imul_offset], ECX
; DECODE 0x75 FOR JNE INSTRUCTION
    XOR     [BYTE EDX+EAX*2 - eax_value*2 + jnz_offset], ECX

    PUSH    ESI                         ; [ESP0] = ESI
    PUSH    ESP                         ; [ESP1] = ESP0
    POP     ECX                         ; ECX = [ESP1] = ESP0
    XOR     ESI, [ECX]                  ; ESI = ESI ^ [ESP0] = ESI ^ ESI = 0
    PUSH    EAX
    POP     ECX
    DEC     ESI                         ; ESI = -1
esi_value   equ -1
...
; bytes will be "patched" before the loop runs to re-create the right bytes.
    INC     ECX                         ; EAX is used as an input index
    INC     ESI                         ; ESI is used as an output index
    ;IMUL EAX, [EDX+ECX*2 - (eax_value+1)*2 + data_offset], 30         ; JNZ offset EF ; Terminating 00
imul: db 0x6B^code, 0x44, 0x4A, - (eax_value+1)*2 + data_offset, 0x30  ; 4A * 30 -> E0 ; 30 * 30 -> 00
imul_offset equ imul - start
    XOR     AL, [BYTE EDX+ECX*2 - (eax_value+1)*2 + data_offset + 1]   ; E0 ^ 45 -> A5 ; 00 ^ 45 -> 45
    XOR     [BYTE EDX+ESI - (esi_value+1) + data_offset], AL           ; 4A ^ A5 -> EF ; 45 ^ 45 -> 00 
