...
decode_loop:
	INC     ESI
	IMUL    EAX, [BYTE ECX + 2*ESI + offset + jnz_marker + 1 - ((esi_value+1)*2)], BYTE 0x30
	XOR     AL,  [BYTE ECX + 2*ESI + offset + jnz_marker + 2 - ((esi_value+1)*2)]
	XOR     [BYTE ECX + ESI + jnz_marker + 1 - (esi_value+1) + 8], AL
